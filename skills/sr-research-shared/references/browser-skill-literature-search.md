...
BrowserSkill 未安装或 `bsk` 命令不可用时：报告具体阻塞原因，按“Windows 安装与连接”章节向用户提示安装（安装脚本、`bsk install-skill --harness codex --json`、浏览器扩展、`bsk doctor --json` 自检），并明确询问用户选择“安装 BrowserSkill 后重试”或“确认切换到公开 API / 人工导出 RIS/BibTeX / 开放全文 / 手工检索”回退路径。未经用户确认不得静默切换回退路径。

daemon/扩展未连接、浏览器不支持、帮助被禁用或站点阻止自动化时，报告具体阻塞并给出修复提示，同样先请用户在“修复重试”与“确认切换回退”之间选择。不得为了“完成命中数”绕过登录、借用确认、验证码、付费墙或站点访问限制。

用户确认切换回退后，按所选回退路径继续，并在成果卡记录用户决策与时间戳。
